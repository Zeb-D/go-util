...
func TestFilter_Int(t *testing.T) {
	...
	for _, tc := range testData {
		mstate := lambda.Filter(tc.expr)
		fmt.Printf("%s", tc.expr)
...
func TestChainable(t *testing.T) {
	...
	ass := assert.New(t)
	for _, tc := range testData {
		mstate := lambda.Filter(tc.f).Map(tc.m)
		fmt.Printf("filter:%s\r\nmap:%s\r\n", tc.f, tc.m)
		r := mstate.Call(tc.dst)
		inf, err := r.Interface()
		ass.NoError(err)
...
func TestThreeChain(t *testing.T) {
	...
	//                                   syntax error
	r := lambda.Filter(`(v) => v % 2 == 0`).Map(`v => v*v`).Call(scores)
	s, err := r.Interface()
	ass := assert.New(t)
	ass.Error(err)
	dst := []int{1, 2, 3, 4, 5, 6, 7}
	r = lambda.Filter(`(v) => v > 3 && v <= 7`).Map(`(v) =>  v << 2`).Filter(`(v) => v % 8 == 0`).Call(dst)
	s, err = r.Interface()
	ass.NoError(err)
